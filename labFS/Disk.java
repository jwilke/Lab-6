 ...
 *
 * (C) 2007,2010, 2011, 2012 Mike Dahlin
 *
 */
...
public class Disk{
	...
	throws IOException
	{
		...
		try{
			...
			assert(!pending.isEmpty() && pending.getFirst().getOperation() != BARRIER);
			//
			// Pull a random-ish item off the list. Use random
			// to test non-fifo list...
			//
			// Note
			//  -- reads and writes cannot pass barrier -- if random item selected is a read/write past
			...
			if(dr.getOperation() == BARRIER){
				dr = pending.removeFirst();
			}
			// Reads and writes cannot pass barrier; If skip takes us to a 
			// read or write write past the barrier,work on first request instead.
			else{
